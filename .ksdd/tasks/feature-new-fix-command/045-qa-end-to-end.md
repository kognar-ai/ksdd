...
## Objetivo
Validar toda a feature (2 commands + template + wiring + integrações) contra os critérios de aceite, cobrindo modos, edge cases e instalação nos 4 targets. Produzir um QA-REPORT.md como os das features anteriores.
...
- **L. Instalação 4 targets** — `install`/`install --codex --opencode --antigravity` copiam `ksdd:new:fix`/`ksdd:build:fix` (e basenames `ksdd-*`) + `fix-template.md`; `status` conta; `uninstall` limpa; idempotência.
...
- [ ] Smoke de install/uninstall nos 4 targets com tmpdir (`*_HOME` override) sem sujar o home real.
