...
`references/parallel-build.md` (feature parallel-build-sync, v0.12.0) segue o mesmo princípio pela via de **reference**, não de command: é **auto-bundlado** pelo `copyDir` de `references/` para o bundle de skill de cada target, **sem** entrada nova em `COMMAND_FILES` nem qualquer alteração em `bin/ksdd.js` (ver ADR-014). `build:feature` e `build:all` apenas o **referenciam** como fonte única do modelo de build paralelo.
...
### ADR-014: Build paralelo (teammates + worktrees), PR único ao final e sync pós-build — conteúdo Markdown, sem tocar `bin/ksdd.js`

**Evidência:** feature parallel-build-sync (v0.12.0) enriquece `/ksdd:build:feature` e `/ksdd:build:all` em quatro eixos — (1) **máximo de paralelismo** (um teammate por task independente, todas as chamadas de agente na mesma mensagem — contrato do skill dispatching-parallel-agents), (2) **isolamento em git worktrees** por teammate paralelo (contrato do skill using-git-worktrees), (3) **PR único ao final** de um build completo, e (4) **fase de sincronização pós-build**. Tudo entregue como **conteúdo Markdown**: reescrita de seções dos dois commands (`commands/build:feature.md` §5/§8.5/§9, `commands/build:all.md` B.4) + um novo doc canônico `references/parallel-build.md`. **Nenhuma alteração em `bin/ksdd.js`** — o reference é auto-bundlado pelo `copyDir` de `references/` (`bin/ksdd.js:168/205/235/283/367`), sem nova entrada em `COMMAND_FILES`; smoke-test confirmou a distribuição do arquivo aos bundles dos **5 targets**. Decisão e trade-off em `.ksdd/features/FEATURE-parallel-build-sync.md` seções 2.1, 6.2 e 9.2.
**Decisão:** tratar a estratégia de build paralelo como **conteúdo distribuído** (mesma filosofia de ADR-003/ADR-013), com `references/parallel-build.md` como **fonte única** — referenciada, não copiada — pelos dois commands. Três decisões de produto ficam registradas: (a) **sync "só docs derivados"** — a fase pós-build faz edição cirúrgica apenas em `README.md`, `CLAUDE.md`/`AGENTS.md`, `CHANGELOG.md` e no tracking de tasks (`status`/README de tasks); `SPEC.md`, `architecture.md`, `DESIGN.md` e `FEATURE-*.md` permanecem **read-only**, com drift apenas **sinalizado** (nunca editado automaticamente); (b) **fallback seguro** — worktree negado pelo ambiente (sandbox) **ou** overlap de arquivos entre tasks da onda ⇒ **execução sequencial in-place** na branch de build, preservando gates/commits/sync/PR único, com aviso amarelo; (c) **PR único como default** do build completo (agrega os commits atômicos das tasks + o commit de sync), com `--multi-pr` opt-in para reproduzir o histórico de 1 PR por task. Como é 100% conteúdo, **não** cria função `install*` nem incorre na dívida do ADR-010/011/012; o gatilho do ADR-012 (**refator `installTarget(targetConfig)` inescapável antes do 6º target**) permanece **intocado** por esta feature.
**Confiança:** alta — decisão explícita do mantenedor no checkpoint da feature (paralelismo + worktrees + PR único + sync como conteúdo; escopo "só docs derivados"; bump minor v0.12.0).
**Consequência:** os dois commands passam a compartilhar **uma única fonte de verdade** (`references/parallel-build.md`), o que **previne a divergência `build:feature`↔`build:all`** que existia (o `build:all` replicava o fluxo antigo de PR-por-task sequencial). A superfície de instalação não muda: a contagem de funções `install*` permanece em **5** e a de slash commands em **11** (nenhum command novo). Débito: a prosa dos commands cresce (paralelismo, worktrees, fallback, sync), mitigado por **referenciar** o doc canônico em vez de copiá-lo; e o build completo ganha um novo checkpoint humano (a sync pós-build). Riscos operacionais novos (worktree negado, conflito de merge entre teammates, drift de docs, PR único grande) ficam registrados na seção 11.
...
| Ambiente (sandbox) nega criação de `git worktree` no build paralelo (ADR-014) | Médio | Média | Fallback seguro: execução sequencial in-place na branch de build, preservando gates/commits/sync/PR único, com aviso amarelo (`references/parallel-build.md` §4) |
| Conflito de merge entre teammates paralelos ao integrar na branch de build | Médio | Média | Só paraleliza tasks **sem overlap de arquivos previsto** (overlap ⇒ onda sequencial); teammates editam e retornam, o orquestrador comita **sequencialmente** após a onda |
| Drift entre a implementação e os artefatos-contrato read-only (SPEC/architecture/DESIGN/FEATURE) na sync pós-build | Alto | Baixa | Regra dura "só docs derivados": os read-only nunca são editados — a sync só **sinaliza** o drift (aviso amarelo) e pausa no **checkpoint** de aprovação antes de comitar os docs derivados |
| PR único do build completo grande demais para revisar | Médio | Média | Commits **atômicos** por task dentro do PR + `--multi-pr` como escape hatch (1 PR por task) |
